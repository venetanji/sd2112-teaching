...
Lecturer: giovanni.lion@polyu.edu.hk. Coordinator: zhibin.zhou@polyu.edu.hk, office V502b. TA contact details are on Canvas.
...
1. **Explain**, in non-technical terms, how Machine A (explicit rules) and Machine B (learned patterns) work, how they differ, and where each appears in design tools and products.
...
Dates: confirm against the PolyU academic calendar for 2026/27 and add them to Canvas. Readings and videos are due **before** the class listed.

| Wk | Topic | Key examples and references | In class | Make / due |
|---|---|---|---|---|
| 1 | **Two ways to teach a machine.** Course intro and the journey; a working definition of AI; Lovelace's objection and Turing's question; rules vs examples, with chairs; using AI vs incorporating AI; the designer's turn. | Lovelace Note G (1843); Turing (1950); Nake, *Homage to Paul Klee* (1965); Rosch & Mervis (1975) typicality; Photoshop's Auto Levels vs Content-Aware Fill vs Generative Fill; Netflix artwork personalisation; Coca-Cola's 2024 generative holiday ad; the Humane AI Pin; Verbeek (2015). | ClassPoint: level and expectation check (studio, AI use, one hope and one worry). Activity **Push the machine to the edge**: 1-2-4 on a cup — each round ends in a prompt, tested on PolyU GenAI (Flux or Qwen), and what you submit to ClassPoint is an image. | Set up ClassPoint, Canvas and a p5.js editor account; log into PolyU GenAI. Watch *AlphaGo* and the four short art films on the playlist before week 3. |
| 2 | **Rules that make things.** Symbolic AI (machine A): if-then, Kant's analytic and synthetic judgements, Turing's imitation game, ELIZA running Weizenbaum's 1966 script, expert systems, the knowledge bottleneck; instructions as art: Young, Fluxus scores, Cage, Tinguely, Kaprow, Sol LeWitt's wall drawings, Molnár; Bense's information aesthetics; Nees's *Schotter* and Nake's *Walk-Through-Raster* broken down into their rules and drawn step by step; 10 PRINT; fractals (Koch's curve, the Mandelbrot set) and L-systems; parametric design (Grasshopper, variable fonts). p5.js as a sketchbook that executes. **Specs and prompts for coding:** a rule written in words (rule, chance, numbers, constraints, output), executed by a language model into p5.js; what you said vs what you meant. | Young, *Composition 1960 #10*; Brecht, *Drip Music* (1959); Cage, *Water Walk* (1960); Tinguely, *Homage to New York* (1960); Kaprow, *Fluids* (1967); LeWitt, *Wall Drawing 118* (1971) and "the idea becomes a machine that makes the art" (1967); Nees, *Schotter* (c. 1968); Nake, *Walk-Through-Raster* (1966); Illiac Suite (1957); *10 PRINT* (1982); Weizenbaum's ELIZA (1966); MYCIN and XCON. | p5.js in the slides: Schotter, Walk-Through-Raster, Koch's curve and LeWitt's points as editable sketches with sliders — change a number, see the picture change. Exercise **One spec, one twist**: LeWitt's Wall Drawing 118 (ten points) executed by a language model on PolyU GenAI into p5.js, then one twist of the pair's own, iterated on the spec rather than the code; one image per pair into ClassPoint, the spec as the caption. | **Challenge 1:** a picture from rules — one rule, one random number, the spec, the code and a screenshot on Canvas. Awards in week 3. |
| 3 | **Learning from examples.** Theories of concepts: the classical theory (definitions; Plato's Meno; Wittgenstein's games) vs prototype theory (Rosch's fruit, typicality); the birth of AI twice: Dartmouth 1956 (GOFAI) and Rosenblatt's perceptron 1958; Hinton, backpropagation and connectionism; parallel calculation, the GPUs made for games, AlexNet; modern AI 2012–2026 in one timeline; combining concepts (pet fish) and conceptual blending. | Plato, *Meno*; Wittgenstein (1953) §66–67; Rosch (1975); Rosch & Mervis (1975); the Dartmouth proposal (1955); Rosenblatt (1958); Minsky & Papert (1969); Rumelhart, Hinton & Williams (1986); Krizhevsky, Sutskever & Hinton (2012); *AlphaGo* (film); Osherson & Smith (1981); Fauconnier & Turner (2002). | Challenge 1 awards. ClassPoint: name a fruit (the room's prototype); quick checks. p5.js in the slides: a perceptron finding its own line; the Mandelbrot rule painted with one core or four thousand. Activity **The blend**, on PolyU GenAI with the image editors that take references (Flux, Qwen Image Edit): alone, a concept as a picture, uploaded with the concept as a caption of 50 characters or fewer; in pairs, two concepts blended into one picture, up to three reference images. | **Challenge 2:** a picture from text and references — two or three concepts of your own blended by an image editor from a prompt and one to three reference images of your own. |
| 4 | **Language machines. From rules to agents.** Recap Machine A and Machine B; Chomsky's generative grammar and Universal Grammar as a theory of human language acquisition; ELIZA's written patterns; RNNs, transformers, parallel training and next-token generation; reasoning, demonstrations and reinforcement learning; question answering and retrieval; agent loops and tools; harnesses as a place where Machine A and Machine B cooperate. Fluent mistakes as a reason to check evidence. | 3Blue1Brown, *LLMs explained briefly*; Ouyang et al. (2022), instruction following with human feedback; DeepSeek-R1 (2025), reasoning through reinforcement learning; Weizenbaum (1966), “ELIZA”; Vaswani et al. (2017), “Attention Is All You Need”; Yao et al. (2022), “ReAct”; Anthropic, “Building Effective AI Agents.” | Evidence-checking warm-up; interactive sentence generator and Transformer; three model-authored slides on what an LLM is and how to collaborate. Gio demonstrates two or three agent harnesses, then small groups brainstorm an AI product or service: choose a person and need, explore three ideas with an agent, prompt it as a skeptical reviewer, and revise one sketch in response to a credible criticism. | **Challenge 3:** ask a language model to draft a brief for a small design task; preserve the prompt and unedited draft, edit it yourself, and explain one important change. Bring the evidence to week 5. |
...
| 20% | Individual reflection | *The role of AI in your creative process, with particular attention to the difference between Machine A and Machine B.* About 1000 words. Evidence: at least three of your own experiments from the weekly challenges (weeks 2–6), with images. Ends with a short process note on how you used AI to write it. Submitted on Canvas. | Week 7 |
...
- **Accessibility.** Slides are published as an HTML deck and on Canvas before class; recordings and captions per school practice. Tell the team early about anything we can adapt.
...
Tools you will need an account for: ClassPoint (student app), Canvas, PolyU GenAI (`genai.polyu.edu.hk`, your PolyU login: it covers image generation with Flux and Qwen, and language models). Adobe Firefly or Bing Image Creator work as alternatives. Everything else is provided in class.
