...
| 2:44 | 69–70 | Challenge 1, homework | The brief; the spec, the code and a screenshot on Canvas before week 3; AlphaGo and the four films. | | TAs stay 30 minutes. |
| 2:42 | | Buffer | | | |
...
The own rule is the start of Challenge 1 (slide 69): a picture from rules, one rule and one random number, the spec plus the code as a text file plus a screenshot on Canvas before week 3; the model is allowed and must be named; the room votes in week 3.
