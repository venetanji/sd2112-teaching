...
3. Explain two rules step by step, with the picture drawing itself: Schotter (row by row, two sliders) and Walk-Through-Raster (the chain written into the raster, cell by cell), then the code of each. Bring back the fractal part of the 2025 deck: Koch's curve, a rule that calls itself.
