...
| Gio | Fonts on the classroom PC (Inter and JetBrains Mono ship inside the deckgen package; README, *Classroom checklist*). The week-1 data: a screenshot of the slide-19 hopes and worries (for slide 5) and the cup wall (for slide 6). Backup: the html deck on a laptop — press `S` for notes; the sketches on slides 30, 31, 42, 44 and 52 run live there, which is the fallback if the p5 editor is slow. |
