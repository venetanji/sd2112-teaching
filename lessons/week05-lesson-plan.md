...
| Gio | Fonts on the classroom PC (`tools/fonts/`). The week-4 data: the capture-2 wall and the Challenge 3 entries for slides 7–8. Decide before class which of the cut-if-behind slides to keep (6, 15, 21, 36, 37). Backup: the html deck on a laptop — press `S` for notes; the sketches on slides 12, 19, 24 and 34 run live there. |
...
| 2:07 | 53–61 | **Activity: the silent decisions** | Section (1 min); round 1 alone, brief only (5); capture 1, everyone (3); round 2 in pairs, two references (8); pulse (2); round 4 in fours, three iterations and the spec (10); capture 2 and the wall (5); *which relation did your layout tool create?* (3); what just happened (2). | 2 × image upload; multiple choice; short answer | Nicolò keeps time; the other three TAs walk with the four questions. Details below. |
| 2:46 | 62–64 | Challenge 4, homework | The brief: the layout, the spec, the critique, on Blackboard before week 6; AltexSoft; a reflection draft; headphones. | | TAs stay 30 minutes. |
| 2:50 | | Buffer | | | |
...
4. **Pulse (slide 57).** *Compared with round 1, the references…* changed the layout and we can say which did what / changed the surface, not the geometry / were mostly ignored / not back yet. No correct answer; A is hoped for, B is the common honest one. Ask a pair who chose C what words they used. Ds get a TA.
...
6. **Capture 2, 5 minutes — scribes only, caption required (slide 59).** One image per four. Caption: the three changes, then "silent decision: …" — kept, or undone. Put the wall on screen next to capture 1; read two captions and have the room find the decision on the picture before it is named. Download the submissions: they seed Challenge 4 and are evidence for the reflection.
...
| 57 | Multiple choice | Compared with round 1, the references… | Pulse on the references; A hoped for, B honest, C means the words were vague. |
...
All nine buttons are generated by the build (caption required on 59).
...
- The capture-2 submissions (slide 59) with their captions: the Challenge 4 seeds, the week-6 awards shortlist, and reflection evidence.
