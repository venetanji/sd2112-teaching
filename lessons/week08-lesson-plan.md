...
**AI as design material** · three-hour lecture-workshop (about 155 minutes of content plus a 15-minute break, with a buffer) · deck: `week08-classpoint.pptx` (58 slides, from the *Build PowerPoints* workflow artifact or a local build) · web: `venetanji.github.io/sd2112-teaching/week08/` (html deck with the three p5.js sketches running live; `SD2112-week08.pdf` next to it)
...
5. Set up six weeks of teamwork: a sprint per week, a backlog, a fifteen-minute stand-up at the team table before every class (the TAs are in the room from half an hour before), the review with the TAs after class, versions and branches for design files, five roles — and start the group proposal in class so that no team goes home with a blank page.
...
| 0:11 | 7–14 | Using, or incorporating | The distinction turned around; the five parts (figure); **live: the threshold** (two mistakes, the counters); every threshold makes two mistakes; **live: personalisation** (the dial, keys 1–4); *you design the space of screens*; *who decided 0.7?* | Multiple choice | Slides 9 and 10 are the core of the first half: walk the figure left to right, then move the line. Cut 12 to one sentence if behind. |
...
| 1:41 | 37–42 | Working as a team | Scrum in three words; the semester as sprints (figure); versions and branches (figure); the process is graded; five roles, named at the table in sixty seconds (42). | | Say "nothing is work unless it is on the list" twice. The stand-up is before class from week 9; the review with the TAs is after class from today. Cut 40 to its caption if behind. |
...
| 2:04 | 47–55 | **Activity: ten ideas, one decision** | Section (1 min); round 1, ten ideas (5); *how many?* (2); round 2, pick one (3); round 3, the card (10); upload, one per team (4); the gallery, four cards (5); the vote (2); *the data you do not have* (3). | Multiple choice; image upload; multiple choice; short answer | Nicolò keeps time; the other three TAs walk. Details below. |
| 2:39 | 56 | Debrief | Every product here is a mediation: the loop, Verbeek, the fifth row. | | |
| 2:43 | 57–58 | Homework | The proposal on Blackboard before week 9; Coded Bias; the first stand-up, at the team table before next week's class. | | TAs stay 30 minutes: the first review. |
| 2:47 | | Buffer | | | |

If the room is slow to form teams, take the time from chapter 3 (one sentence each for Rabbit and the cautionary cards) rather than from the activity. If the room is fast, let two teams read their card aloud before the gallery.
...
3. **Teams, 3 minutes — pick one (slide 50).** Read the ten aloud; apply the three tests on the panel (does the data exist? is the failure survivable? can you wizard it by week 10?); a line that fails one is out. Tie-break: the clearest sentence at rung three. Circle it. A pick, not a marriage: week 10 can change it. Rounds 2 and 3 share one submission, the card on slide 52; a vote here would be a tenth question.
4. **Teams, 10 minutes — the decision card (slide 51).** One A4, landscape, big letters: team number; the decision in one sentence; for whom (one person, named like a character); the data in two columns, *have* and *do not have*; if it is wrong — what she sees, what she can do, the fallback. If the honest answer to row five is "nothing", write "nothing". This is the proposal in draft.
...
The card is the draft of the group proposal (slide 45): typed up, with two extra lines (rule or model, and why; the team and the five roles), one page on Blackboard before the week-9 class. Amber reads every proposal before week 9, and week 9 opens with them (its slide 7); the TAs' thirty minutes after class are the sprint review, from today.
...
| 14 | Multiple choice | A product shows a feature only above a score of 0.7. Who decided 0.7? | Quick check: B — someone chose it, and can change it. Closes chapter 2. |
