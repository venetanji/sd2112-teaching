...
| Who | Task |
|---|---|
| Nicolò | Classroom PC: ClassPoint open, `week03-classpoint.pptx` loaded, join code on screen from slide 1. Fire the slide-4 word cloud once as a test and reset it. Check the two Image Upload buttons (slides 57 and 59, both *caption required*). Open the html deck in a browser tab as well: slides 26, 28, 34 and 41 run the sketches live there, and that is what the room sees for them. |
...
| 1:03 | 31–37 | Neurons that learn: Hinton | Hinton, 1986 to 2024; backpropagation in words (33), then live (34: the guess goes forward, the error comes back, every weight moves, and the crosses turn into ticks); two theories, two machines; the deal (fluent, fuzzy, opaque); the sentence to carry across the break. | | Slide 35 is the bridge of the module and the map for the reflection: the figure fills the slide, the two sentences are in the notes. |
...
In the html deck, the code panel of slides 28 and 41 is an editor: **Run** (or ctrl+enter) re-runs the sketch beside it with whatever is in the box, **Reset** brings the slide's code back, and what a student typed survives a reload. On 28 the sketch is the perceptron: it settles from a reload, a click adds an example of class A, a shift-click one of class B, and C starts again; the bar under it shows the three numbers. Slide 27, just before it, says in four cards what the code is trying to do (the examples, the rule, the nudge, the clicks), so read that one before pressing anything. On 41 the slider is how many pixels are painted in each frame, 2^k from 1 to 4,096, and a click starts the painting again from the top. The PowerPoint and the PDF show the code and a still, and their captions link to the slide in the html deck. Slides 26 and 34 are figures that run live in the html deck (a click pauses, C starts again, a slider sets the seconds per example). On 26 the neuron of the figure meets twelve cups and twelve bowls one at a time, and its three numbers move whenever a guess is wrong. On 34 a network of nineteen units learns to tell a 3 × 3 chair from a 3 × 3 cup: the guess travels forward as black dots, the error back as orange ones, every connection flashes as its weight moves, and the top right counts how many of the last twenty it got right.
