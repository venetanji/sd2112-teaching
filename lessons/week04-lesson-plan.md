...
| 0:30 | 14–18 | Machine B · learned sequences | RNN state; RNN vs transformer; tokenisation, embeddings, attention and next-token generation; base vs instruction-tuned models. | | On slide 16, click several token positions to show which earlier tokens each can use, then replay the parallel training pass. Contrast this with slide 15's recurrent chain. Ask students to narrate one generation loop from prompt to stop token. Mention hallucination and sycophancy as reasons to check claims against evidence. |
