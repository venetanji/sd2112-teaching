...
S.append(figure_slide('02 · FOUR ROUNDS', 'Half present. Half visit. Then swap.', F.w13_rotation(),
                      ...
                      notes='Read the rows top to bottom. Half A presents in rounds 1 and 3, half B in 2 and 4, so everyone presents twice and visits twice. The graders\' row: five of us, two or three boards each a round, about four minutes a board, a band and a line per criterion; the second pass sends a different one of us to every board, so every team ends the fair with two sheets from two people. The ClassPoint row: one activity, open the whole time, one line per poster visited. Say the visitor\'s pace out loud — three minutes a poster, then move, four posters a round — and that the presenter\'s job is to answer the five questions on the next slide, not to give a speech.'))
