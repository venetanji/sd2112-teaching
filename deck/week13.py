...
clock (rounds, halves, a drawn bell) and the thirteen weeks on the rules ↔ examples line; the fair
clock, set to thirty-five minutes, also sits on the quiz placeholder slide.
"""
import sys
...
// Keys: + and - change the length of a round, [ and ] the number of teams. Every other key goes to the deck.
...
function draw() {
  ...
  text(MODE == 'fair' ? '+ / -: minutes    [ / ]: teams    click: start · pause · next round' : '+ / -: minutes    click: start · pause', 470, 452);
  if (over) bell(1300, 110, frameCount % 40 < 20);
...
QUIZ_CLOCK_JS = ROUNDS_JS.replace("const MODE = 'fair';", "const MODE = 'quiz';").replace('let minutes = 12, teams = 26;', 'let minutes = 35, teams = 26;')
...
const X0 = 90, X1 = 1310, Y = 392;                                     // the axis
let tw;
...
S.append(question('word_cloud', 'Thirteen weeks in one word.',
                  ...
                  notes='ClassPoint word cloud, one word each, ninety seconds. Put the week-1 cloud next to it if you have the screenshot: in week 1 the big words were tools, speed, jobs, cheating, images. Read the three biggest words now and say what changed. Keep the screenshot: it goes in the last slide of the course next year. Then the three parts.'))

S.append(cards('01 · THREE PARTS', 'Three parts. Then it is done.', [
    ('THE FAIR · 40 %', 'Your project, on the wall.',
     ['Four rounds of twelve minutes. Half the teams present while the other half visits; then they swap, twice. One line of feedback per poster you visit, on ClassPoint. The five of us walk with a sheet per team.',
      'Ends with the peer prize: the poster you would give it to, and a vote.']),
...
], text_size=22, notes='Three parts, said once, so nobody asks at 2:30 when the quiz is. The fair is sixty percent of the morning and forty percent of the mark; the quiz is twenty. Say the rule of the day: you present twice and you visit twice, and every visit ends in one line on ClassPoint. Then the room.'))

S.append(figure_slide('01 · THE ROOM', 'Boards on the walls. Two halves. One clock.', F.w13_room(),
                      body=['Twenty-six boards on three walls, numbered as on the Blackboard group list: half A is teams 1 to 13, half B is 14 to 26. A laptop with the video at every board, the clock on the screen, the desk by the door.'],
                      ...
                      notes='Walk the plan with a finger. Left wall and the first half of the back wall: teams 1 to 13, half A, presenting in rounds 1 and 3. The rest: half B, rounds 2 and 4. The screen shows the clock and, behind it, the feedback wall growing. The desk is where the graders\' sheets live and where a dead phone gets a charger. Ask each team to look at its number now and to decide who stands at the board in round 1 — and who in round 3, because it should be a different person.'))
...
S.append(figure_slide('02 · FOUR ROUNDS', 'Half present. Half visit. Then swap.', F.w13_rotation(),
                      body=['Twelve minutes a round: a visitor sees four to six posters, three minutes each, and writes one line per poster. The second pass is not a repeat: you go to the posters you missed, a different person talks at the board, and the graders finish their sheets.'],
                      ...
                      notes='Read the rows top to bottom. Half A presents in rounds 1 and 3, half B in 2 and 4, so everyone presents twice and visits twice. The graders\' row: five of us, a sheet per team, a line per criterion; in the second pass we go back for a second opinion where the first was thin. The ClassPoint row: one activity, open the whole time, one line per poster visited. Say the visitor\'s pace out loud — three minutes a poster, then move — and that the presenter\'s job is to answer the five questions on the next slide, not to give a speech.'))
...
S.append(figure_slide('02 · THE FORTY PERCENT', 'How the forty percent is decided.', F.w13_grading(),
                      body=['Four things are read: the poster at the fair, the video on the laptop, the brief and the log on Blackboard. Five criteria from the syllabus. Five of us with a sheet per team today; a grading meeting after; the mark and a paragraph per team on Blackboard.'],
...
S.append(sketch_slide('02 · LIVE · THE FAIR CLOCK', 'Round 1. Teams 1 to 13 present.',
                      live('w13-rounds', ROUNDS_JS, 1400, 500, hint='click = start · pause · next round  ·  + and − change the length  ·  [ and ] the number of teams'),
                      body=['Twelve minutes a round, four rounds. The clock says which half presents and which half visits; at zero the ring turns red and a bell appears — it is drawn, not rung: the TAs call the swap. Two minutes to swap, then the next round.'],
                      caption='Nicolò runs it from the html deck on the second screen; the PowerPoint keeps the feedback activity open. The number of teams and the length of a round are set on the day with the keys.',
                      notes='Nicolò clicks when the first presenters are at their boards. Round 1: half A at the boards, half B walking with phones. At the bell: two minutes, everyone swaps, click, round 2. Rounds 3 and 4 are the second pass — say it again when round 3 starts: go to the posters you missed, and a different person talks. If the room is behind, press minus twice for rounds 3 and 4 (ten minutes); if it is ahead, do not add time — start the prize early. The graders keep moving; if a board has no visitor for a minute, a TA sends one.'))

S.append(question('image_upload', 'Everyone: the poster you would give the prize to.',
                  hint='One image: the poster, photographed. Caption: the team number, nothing else. Not your own team.',
                  ...
                  notes='Four minutes, everyone, one image each, caption required and the caption is a team number. Names hidden, so the wall is posters and numbers. Amber counts the captions as they come in — the four most-uploaded teams are the finalists, and she writes their numbers on the next slide before the vote. Download the responses after class: the photographs are the only pictures of the fair we keep, and none of them shows a face if the teams photographed the board.'))

S.append(question('multiple_choice', 'The peer prize: which of the four?', [
    'Finalist A · team number on screen', 'Finalist B · team number on screen', 'Finalist C · team number on screen', 'Finalist D · team number on screen',
], eyebrow_text='02 · THE PEER PRIZE · MULTIPLE CHOICE · ONE VOTE EACH',
    notes='Amber reads the four team numbers and points at the boards; the room votes once, not for its own team. No correct answer: the room decides, the winning team gets a participation star each, the same as a challenge award, and thirty seconds at the goodbye to say what their model decides. Show the split and thank all four; then the break. The prize is peer recognition, not a grade — say so, because the rubric decides the forty percent.'))
...
S.append(question('short_answer', 'Can a machine originate a design? Yes or no — and why.',
                  hint='One line. Week 1 asked it with Lovelace; week 12 asked it again. This is the last time. Names are hidden; the vote is not graded.',
                  eyebrow_text='03 · THE COURSE QUESTION · SHORT ANSWER · NAMES HIDDEN',
                  cp={'type': 'short_answer', 'hide_names': True, 'multiple': False},
                  notes='Three minutes, anonymous. Read six lines, two from each side, and count the room roughly by hand: yes, no, it depends. Put the week-12 answers next to it if Amber has the screenshot, and the week-1 hopes-and-worries wall if you want the long view: in week 1 nobody had the vocabulary; now the "why" clause has the words in it — the middle, the pick, the guardrail. Do not give your own answer yet; the next slide has the arguments, and my answer is the last sentence of the notes there.'))
...
S.append(sketch_slide('04 · THE QUIZ · 35 MINUTES', 'Quiz: open the ClassPoint quiz mode.',
                      live('w13-quiz-clock', QUIZ_CLOCK_JS, 1400, 500, hint='click = start · pause  ·  + and − change the length'),
                      body=['The questions are in the quiz deck, not in this one. Nicolò opens it with the ClassPoint add-in; this clock stays on the second screen. Blackboard fallback: the test opens at my word and closes itself thirty-five minutes later.'],
...
S.append(end('That was SD2112. Thank you.',
             'Marks on Blackboard after the grading meeting. The site stays.',
...
# LeWitt: https://www.kim-cohen.com/Assets/CourseAssets/Texts/LeWitt_Paragraphs%20(1967).pdf and https://monoskop.org/images/3/3d/LeWitt_Sol_1967_1999_Paragraphs_on_Conceptual_Art.pdf
...
# Ihde and Verbeek:
#   https://link.springer.com/article/10.1007/s13347-014-0149-8  (Ihde's four relations — embodiment, hermeneutic, alterity, background — from
