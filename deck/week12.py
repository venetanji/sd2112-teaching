...
S.append(sketch_slide('02 · ELIZA · LIVE · FOURTEEN RULES OF OUR OWN', 'Talk to it. Watch the rule that decided.',
                      ...
                      notes='Type three lines from the room. "I need a logo for my café": rule 1 fires before rule 7 — order is rank. "It is fine": nothing fires, and MEMORY brings back the "my". Then a line that breaks it: "Can you send it to my client by Friday?" fires rule 5 and the reply is nonsense, because it never understood anything. Every reply has a line you can point at. That is the deal from week 2, and it is what the generative version gives up.'))
...
S.append(cards('04 · WHAT PASSING MEANS WHEN EVERYONE PASSES', 'The test moved from the lab to the law.', [
    ...
    ('2025 · THE RESULT', 'A persona prompt beats the person.', 'GPT-4.5 told to adopt a humanlike persona was chosen as the human 73% of the time — more often than the real people it sat beside. Without the persona, 21%. The interrogators lost to a prompt, not to a mind.'),
...
], text_size=22, notes='Say it plainly so nobody revises the wrong thing: mechanisms and the two machines, not dates; the two readings; the cases. The notes in the html deck have every quick-check answer, and that is deliberate. The TAs run a revision hour before week 13’s class; time on Blackboard.'))
...
S.append(cards('09 · DUE · WEEK 13 · THE FAIR', 'Print it. Upload it. Revise.', [
    ...
    ('THE QUIZ', 'Revise the whole course.', 'Multiple choice, after the fair, in the same class. The decks and their notes are on the site; the quick checks are the format. The TAs run a revision hour before class.'),
