...
|---|---|---|
| **Publish site** (`.github/workflows/site.yml`) | The html decks, the syllabus page, the landing page, and a PDF of each deck without the ClassPoint buttons, printed one deck per job; a deck whose inputs did not change is restored from the Actions cache, not printed again. | GitHub Pages: `https://venetanji.github.io/sd2112-teaching/` (Week 1: `/week01/`, PDF: `/week01/SD2112-week01.pdf`). |
...
| **Build staging** (`.github/workflows/staging.yml`) | Everything above, from the `staging` branch: html decks, PDFs, PowerPoints, documents, previews. | The `staging-site` artifact of the run (kept 90 days) and `staging-powerpoints` (30 days); *Publish site* re-runs and shows `staging-site` under `https://venetanji.github.io/sd2112-teaching/staging/`. |

## Staging: check a week before it reaches main

Push (or merge) to the `staging` branch. **Build staging** builds everything and fails the way `main` would (a text overflow, a sketch with no still); its summary links the two artifacts. When it ends, **Publish site** re-runs from `main` and republishes the Pages site with that build under `/staging/`: the html decks and PDFs from the run's `staging-site` artifact, built with the staging branch's own `requirements.txt`, so a toolchain change can be checked there too. The published decks are untouched. A broken staging build never blocks `main`: `/staging/` is always the last successful *Build staging* run's artifact (kept 90 days), so the preview does not disappear. When it looks right, merge to `main` (the feature branch, or `staging` itself).
...
Copy the previous week's source to a new `deck/weekNN.py`, change `FOOTER` and `pdf`, write slides with the layout functions in `deckgen.layouts` (`title`, `agenda`, `section`, `statement`, `quote`, `content`, `cards`, `question`, `image_full`, `timeline`, `journey`, `activity`, `video`, `assessment`, `team`, `two_col`, `figure_slide`, `code_slide`, `sketch_slide`), add the week to `decks` and `nav` in `deckgen.toml`, and add a card and PDF link in `site/index.html`. ClassPoint activities come from `question(...)` (word cloud, multiple choice, short answer, image upload) or an explicit `cp={...}`.
