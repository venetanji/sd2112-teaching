...
|---|---|---|
| **Publish site** (`.github/workflows/site.yml`) | The html decks with the live sketches, a PDF of each deck without the ClassPoint buttons, the sketchbook page, the syllabus page, the landing page. | GitHub Pages: `https://venetanji.github.io/sd2112-teaching/` (Week 1: `/week01/`, PDF: `/week01/SD2112-week01.pdf`, sketches: `/sketches.html`). |
...
| Path | What |
|---|---|
| `deck/week01.py` … `deck/week13.py` | **The slides, one Python spec per week**: text, speaker notes, ClassPoint activities, live p5.js sketches (as strings), in the order the class runs, with a `# Sources` block at the end. Edit here; every output updates. |
...
| `lessons/weekNN-lesson-plan.md` | Run of show for each three-hour class, the TA tasks before class, the activity in detail, the ClassPoint question map, what to keep after class. For the teaching team: `.docx` and `.html` in the artifact, **not published**. |
| `site/` | The site shell: landing page, vendored reveal.js and the ait4x design tokens. |
| `tools/deckgen.py`, `layouts.py`, `figures.py`, `figures_weekNN.py`, `course.py` | The generator: a 1920×1080 layout engine with html, pptx and png backends, the ait4x layouts (plus code, sketch and activity slides), drawn figures shared (`figures.py`) and per week (`figures_weekNN.py`), and the shared semester map, week titles and links. |
...
python deck/week01.py                     # only the Week 1 deck (same for every weekNN.py)
python tools/snap.py week03 --force       # redo the stills of week 3's live sketches (deck/assets/sketches, committed)
