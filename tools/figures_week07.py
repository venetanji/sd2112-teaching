...
def w07_decision_loop(name='w07-decision-loop', w=1680, h=560):
    ...
    _lines(c, 322, 94, ['taken, given, inferred', 'consent · minimisation'], size=16, color=MUTED, lh=20, anchor='middle')
    c.line(322, 124, 322, y - 14, ORANGE, 2, cap='butt')
    c.text(905, 70, 'WHERE IS IT BIASED?', size=18, color=ORANGE, anchor='middle')
    _lines(c, 905, 94, ['data · label · algorithm · interaction', 'who is missing from the examples'], size=16, color=MUTED, lh=20, anchor='middle')
    c.line(905, 124, 905, y - bh / 2 - 6, ORANGE, 2, cap='butt')
    c.text(1087, 300, 'THE GUARDRAILS', size=18, color=ORANGE, anchor='middle')
    _lines(c, 1087, 324, ['what it may not decide', 'when a human is in the loop'], size=16, color=MUTED, lh=20, anchor='middle')
    c.line(1087, y + 14, 1087, 278, ORANGE, 2, cap='butt')
    c.text(1320, 400, 'WHICH RELATION?', size=18, color=ORANGE)
    _lines(c, 1320, 424, ['through it · off it · facing it', 'around them · how hard it pushes'], size=16, color=MUTED, lh=20)
...
def w07_brief_anatomy(name='w07-brief-anatomy', w=1680, h=560):
    ...
    for i, (head, q, lines, wk, tint) in enumerate(sections):
        ...
        _lines(c, ax, y + 54, lines, size=17, color=MUTED, lh=22)
...
def w07_team_skills(name='w07-team-skills', w=1680, h=560):
    ...
    c.text(w - 20, 34, 'five illustrators: a poster and no brief · five coders: a demo and no poster', size=16, color=MUTED, anchor='end')
