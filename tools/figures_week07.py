...
def _box(c, x, y, w, h, title, lines, fill=PAPER, stroke=None, title_color=INK, size=17):
    c.rect(x, y, w, h, fill=fill, stroke=stroke, width=2)
    c.text(x + 20, y + 36, title, size=19, color=title_color)
    _lines(c, x + 20, y + 68, lines, size=size, color=MUTED)
...
def w07_quiz_flow(name='w07-quiz-flow', w=1680, h=520):
    ...
    for i, (t, lines) in enumerate(lane2):
        ...
        if i < len(lane2) - 1:
            _arrow(c, x + bw + 4, y + bh / 2, x + step - 6, y + bh / 2, INK, 3, 10)
    _lines(c, 20 + 4 * step, 372, ['either lane:', 'the same questions,', 'one attempt,', 'ten percent.'], size=18, color=INK)
...
def w07_decision_loop(name='w07-decision-loop', w=1680, h=560):
    ...
    # data, model, decision
    boxes = [(410, 'THEIR DATA', ['what they do, say, are', 'what they gave, what', 'was taken'], PAPER),
    ...
    for x, t, lines, fill in boxes:
        ...
        c.text(x + 18, y - 16, t, size=20, color=INK)
        _lines(c, x + 18, y + 12, lines, size=15, color=MUTED, lh=20)
    _arrow(c, 240, y, 404, y, INK, 4, 12)
    _arrow(c, 660, y, 774, y, INK, 4, 12)
    _arrow(c, 1030, y, 1144, y, INK, 4, 12)
    # the return: what it does to them
    c.line(1275, y + bh / 2, 1275, 420, INK, 4, cap='butt')
    c.line(1275, 420, 170, 420, INK, 4, cap='butt')
    _arrow(c, 170, 420, 170, y + 70, INK, 4, 12)
    c.text(722, 452, 'WHAT IT DOES TO THEM', size=20, color=INK, anchor='middle')
    c.text(722, 480, 'what they see · what they do · what they stop doing · who they become', size=17, color=MUTED, anchor='middle')
    # the four questions of the brief, as tags on the loop
    c.text(322, 70, 'WHAT DATA?', size=18, color=ORANGE, anchor='middle')
    _lines(c, 322, 94, ['taken, given, inferred', 'consent · minimisation'], size=15, color=MUTED, lh=20, anchor='middle')
    c.line(322, 124, 322, y - 14, ORANGE, 2, cap='butt')
    c.text(905, 70, 'WHERE IS IT BIASED?', size=18, color=ORANGE, anchor='middle')
    _lines(c, 905, 94, ['data · label · algorithm · interaction', 'who is missing from the examples'], size=15, color=MUTED, lh=20, anchor='middle')
    c.line(905, 124, 905, y - bh / 2 - 6, ORANGE, 2, cap='butt')
    c.text(1087, 300, 'THE GUARDRAILS', size=18, color=ORANGE, anchor='middle')
    _lines(c, 1087, 324, ['what it may not decide', 'when a human is in the loop'], size=15, color=MUTED, lh=20, anchor='middle')
    c.line(1087, y + 14, 1087, 278, ORANGE, 2, cap='butt')
    c.text(1320, 400, 'WHICH RELATION?', size=18, color=ORANGE)
    _lines(c, 1320, 424, ['through it · off it · facing it', 'around them · and how hard it pushes'], size=15, color=MUTED, lh=20)
    c.line(1282, 420, 1306, 420, ORANGE, 2, cap='butt')
    c.text(20, 34, 'THE PRODUCT YOU DESIGN: A LOOP, NOT A PICTURE', size=18, color=ORANGE)
    c.text(w - 20, 34, 'the four questions on the loop are the four sections of the mediation brief', size=17, color=MUTED, anchor='end')
    return c.finish(name)
...
def w07_brief_anatomy(name='w07-brief-anatomy', w=1680, h=560):
    ...
    c.text(px + 24, py + 64, '[team] · [product] · one page', size=15, color=MUTED)
    ...
    sections = [('1 · THE RELATION', 'WHICH RELATION ARE YOU BUILDING?',
                ...
                ('3 · THE BIAS', 'WHERE IS IT WRONG, AND FOR WHOM?',
                 ['data bias · label bias · algorithmic bias · interaction bias',
                  'the bias register: who is missing from the examples, who pays for it'], 'WEEK 9', TINT_ORANGE),
    ...
    for i, (head, q, lines, wk, tint) in enumerate(sections):
        ...
        c.text(px + 38, y + 30, head, size=16, color=INK)
        ...
        c.text(ax, y + 24, q, size=19, color=INK)
        _lines(c, ax, y + 54, lines, size=16, color=MUTED, lh=22)
        c.text(w - 20, y + 24, wk, size=17, color=ORANGE, anchor='end')
    c.text(w - 20, 40, 'one page · four answers · every answer has a week', size=17, color=MUTED, anchor='end')
    return c.finish(name)
...
def w07_pitch_anatomy(name='w07-pitch-anatomy', w=1680, h=560):
    ...
    beats = [(0, 10, 'THE PERSON', TINT_TEAL, 'who is in front of it, now?', ['Someone on the 6:40 bus,', 'half asleep, one hand', 'free.']),
             ...
             (30, 40, 'FOR WHOM', TINT_VIOLET, 'who else, who is left out', ['Commuters whose route', 'changes with the weather.', 'Not the tourist.']),
    ...
    for s0, s1, t, tint, q, ex in beats:
        ...
        c.text(bx0 + 4, y - 36, q, size=15, color=INK)
        c.text(bx0 + (bx1 - bx0) / 2, y + bh / 2 + 7, f'{s1 - s0} s', size=20, color=INK, anchor='middle')
        _lines(c, bx0 + 4, y + bh + 40, ex, size=15, color=MUTED, lh=21)
    ...
    c.text(x1, 70, 'the example is made up; the beats are not', size=17, color=MUTED, anchor='end')
    c.text(x0, y + bh + 140, '0 s', size=16, color=MUTED)
    c.text(x1, y + bh + 140, '60 s · the timer does not care. stop.', size=16, color=MUTED, anchor='end')
    c.text(x0 + 30 * pps, y + bh + 140, '30 s', size=16, color=MUTED, anchor='middle')
    c.text(x0, y + bh + 190, 'The room is listening for one thing: a decision a model makes for a person, that you can say in a sentence. If it needs a demo, it is not a pitch yet.', size=17, color=INK)
    return c.finish(name)


# ───────────────────────── teams: five kinds of work, four deliverables ─────────────────────────
def w07_team_skills(name='w07-team-skills', w=1680, h=560):
    """Five kinds of work in a team of four or five, and which deliverable each one feeds."""
    c = Canvas(w, h)
    ...
    deliv = [('THE POSTER · A0', 'research and concept'), ('THE VIDEO · 3 – 5 MIN', 'how it works'),
             ('THE MEDIATION BRIEF', 'relation · data · bias · guardrails'), ('THE PROCESS LOG', 'who did what, when')]
    feeds = {0: [0, 2], 1: [0, 1, 2], 2: [0, 1], 3: [1], 4: [2, 3]}
    wx = [190 + i * 325 for i in range(5)]
    wy = 130
    dx = [270 + j * 385 for j in range(4)]
    dy = 440
    ...
    for i, (t, lines, col) in enumerate(work):
        ...
        _lines(c, wx[i], wy + 116, lines, size=15, color=MUTED, lh=20, anchor='middle')
    for j, (t, sub) in enumerate(deliv):
        c.rect(dx[j] - 170, dy - 40, 340, 80, fill=PAPER, stroke=INK, width=2)
        c.text(dx[j], dy - 6, t, size=18, color=INK, anchor='middle')
        c.text(dx[j], dy + 20, sub, size=15, color=MUTED, anchor='middle')
    c.text(20, 34, 'FIVE KINDS OF WORK · FOUR OR FIVE PEOPLE · EVERYONE DOES TWO', size=18, color=ORANGE)
    c.text(w - 20, 34, 'a team of five illustrators has a poster and no brief; a team of five coders has a demo and no poster', size=16, color=MUTED, anchor='end')
    c.text(w / 2, 536, 'The lines are who feeds what. Mixed skills means every deliverable has at least one line into it.', size=17, color=INK, anchor='middle')
    return c.finish(name)
